...
import bwapi.*;
import bwapi.types.DamageType;
import bwapi.types.Race;
import bwapi.types.UnitSizeType;
...
public class JFAP extends AJFAP{
	...
	public Pair<Integer, Integer> playerScores() {
		int mine = 0;
		int notmine = 0;
		// Was getting the following error by eclipse:
		//The left-hand side of an assignment must be a variable
		// TODO Ask Jabbo or Yegers about this
		for (final JFAPUnit u : player1) {
			mine=mine+score(u);
		}
		for (final JFAPUnit u : player2) {
			notmine=notmine+score(u);
		}
		
		return new Pair<Integer, Integer>(mine, notmine);
	}
